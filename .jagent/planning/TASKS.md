...
- [x] **BUCKETS-12** (M): **HerdController in-process API wiring** — `snapshot`/`stop` wired into deploy shutdown via Arc-share + Arc::try_unwrap, deduplicated inline kill loop. `scale` kept as `#[allow(dead_code)]` (needs IPC for cross-process hot-scale, deferred). Ls now displays instance counts.  _(nara, 2026-07-18)_
...
- [ ] **gap** — CLAUDE.md's Build section says '71 tests' but BUCKETS-12's own commit (7281e75) reports 95/95 tests passing — CLAUDE.md test count is stale, not touched this round (out of BUCKETS-13's scope)  _(nova-buckets, 2026-07-21)_
